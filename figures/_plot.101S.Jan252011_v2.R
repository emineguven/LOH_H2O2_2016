...
filename = '101S.PeroxideEffectonLOH.Jan252011.csv';
fullFileName = paste('../data.H2O2-LOH/',filename, sep='');
mylabel = '101S 2011 Jan 25'
...
b.min = sum(b.tmp[1])/1
b.tmp = rev(b.tmp); 
b.max = 1.1* sum(b.tmp[1:2])/2
  
modelBlack = function(v, w) { b.max - (b.max-b.min)/(1 + (t/v)^w) }
modelBlack1 = function(v) { b.max - (b.max-b.min)/(1 + (t/v)) }
#
#fm.b <- gnls( b ~ modelBlack1(v) , start = list( v = 0.05) ); fm.b
fm.b <- gnls( b ~ modelBlack(v,w) , start = list( v = 0.042, w= 2  )); fm.b
...
fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5(v,w) , start = list( v = 0.009, w=1)  );   fm.b0.5
